...
The most important risks are the owned capture code itself and the unanswered facts: payload sizes, redaction constraints, and the report endpoint's particulars are still open intake items, and every performance number in this report was measured on proxy hardware. The next step is a spike that measures projection cost on the real dispatch path, and the design-level failure-mode enumeration beside this report names the hazards that spike must exercise.
...
5. **The named fallback exists before it is needed**: if the spike's on-device measurement fails the capture-cost budget (the desktop-proxy figures — ~0.44 µs for a ~200 B record, ~17 µs for ~12 KB — de-rate 10× to stay under ~3% of a core at 50 events/s, but the real device class is intake item f), plan B is a capture-turn shallow snapshot (top-level own-property copy with primitives inlined) plus idle-time serialization, which trades a bounded mutation window for capture-turn cost. It is a fallback, not the design, because it re-admits a slice of the mutation hazard.
...
export interface RecorderConfig {
   ...
   *  fleet). Whether deviceId exists for page code is intake item i. */
...
export interface RecorderTap {
   ...
   *  Inside, in order, before return: project → REDACT → bounded-serialize (walk
   *  guarded; app code may throw) → meter UTF-8 → truncate-with-copy → stamp
   *  {seq, gseq, wallMs, monoMs} → evict-oldest to fit (I2/I3). Redaction precedes
   *  serialization (corrected 2026-08-23): key and path rules need the object
   *  graph, and running them over a finished JSON string would either re-parse it
   *  or let a pattern substitution corrupt the body across syntax — the applier is
   *  the serializer's replacer, which is what Q13's "inside the capture-turn walk"
   *  describes. meta carries only facts the tap knows; policy lives in config —
...
const recorder = createFlightRecorder({
  ...
  envelopeBytes: 40 * MiB,                              // I3's operand, not a hidden constant
...
**What each death produces, stated plainly per arm.** In-page errors: `window.onerror` and `unhandledrejection` see them, with one ceiling — cross-origin scripts without CORS attributes yield opaque `"Script error."` events with no stack, so the nets' yield depends on script origins (the app's own bundle is same-origin; third-party scripts, if any, report opaquely). Component-tree errors: React 18.3.1 has **no root-level error hook**, and a boundary-caught error never reaches `window.onerror` in production (development behaviour differs, which hides the gap from tests) — so the charter's "React error boundaries" net is honestly a **recorder-supplied boundary component** the app must mount, enumerated app work rather than an armed hook; the recorder ships `createRecorderBoundary(recorder)`, a factory returning a boundary component that calls `capture()` in `componentDidCatch` and renders its `fallback` prop, and the report's integration checklist counts mounted boundaries as config. Renderer deaths (OOM, unresponsive kills, app-manager kills): no page event fires, ever. On the Chromium arm the compensation is server-side and cheap: crash reports have been deliverable via the `Reporting-Endpoints` response header **since Chrome 96** to the endpoint named `default` — the plan's "Chromium 139+" claim is corrected: Chrome 139 only added the dedicated `crash-reporting` endpoint *name* ([WICG spec delivery rule; Chrome 139 release notes](https://wicg.github.io/crash-reporting/)). The report body carries `reason: 'oom' | 'unresponsive'`, `is_top_level`, and `visibility_state`, POSTed as `application/reports+json` — a backend header change with no client code, recommended if intake item h answers yes. On the Firefox arm there is nothing: Firefox parses `Reporting-Endpoints` only from 130 and no source shows Firefox generating crash-type reports at any version — at the ~124 floor, **crash-shaped deaths on Firefox produce no report, full stop**, and the compensation is absence-based detection at the backend: a device whose bundles and heartbeats stop arriving mid-session is the crash signal, keyed on question 9's device and page-incarnation identity, built by whoever owns the report backend (named as backend work, not recorder work — the recorder's contribution is the identity fields that make the query possible).
...
(All of this lives behind question 6's `transport` seam — it is delivery policy, invisible to the capture surface; the counters return through the seam into `health().delivery`.) Bounded-then-drop beats retry-until-tab-death on this fleet's bias: under a sustained outage the pinned bundle is 10–50 MB of RAM on a RAM-disk device for an indefinite win, and the charter's own safety rule — a recorder must never become the incident — prices pinned memory higher than a late delivery. The evidence gap is declared: intake item l (interruption rates) is unanswered, so the retry horizon is a judgment defaulting safe, revisitable with data. Delivery is **at-least-once**; the report says so to the endpoint's owners (intake item c) with the dedup key from question 9.

**Durable parking fails its burden of proof at survey depth.** The charter set the bar: a hard cap, a real evidence win over the null, no accumulation path under a dead backend. The evidence win is the tab-crash-during-delivery window, and it cannot be quantified — intake item l is open, and "nobody has measured" was pre-declared an acceptable answer that becomes this report's assumption. Against an unquantified win stand three verified costs: on this fleet IndexedDB is RAM (parking buys the tab-crash window only, never a power cycle); a parked queue under a dead backend accumulates against the OS unless capped-and-reaped code exists and is itself correct; and the multi-tab/origin/schema/eviction reservations 0150 owns would all activate for this store too (the tracks share an origin decision the moment both persist). The verdict: **no durable parking in the recommended design**; the revisit trigger is named — if intake item l shows delivery interruption is common *and* 0150's store ships (so the origin/schema decisions are made once), parking a single sealed bundle (cap: one, evict-newest-refused, self-deleting on send) becomes a cheap rider on 0150's machinery. Until both hold, the loss marker (question 9) is the compensation: losses are reported, not silent.

**Delivery independence**: the report POST is a bare `fetch` outside 0060's boundary, deliberately — the failure being reported is plausibly the boundary itself, and the recorder must not depend on the surface it is diagnosing. This does not violate the D-0006 choke-point rule as ratified: that rule governs *contracted partner traffic through the transport boundary*, and the report endpoint is the team's own contract, vendored in this repo, validated at the schema level by 0010's generated validators **at build/test time** (the bundle is validated against the generated schema in tests and in the spike; runtime validation of an outbound diagnostic POST adds cost in exactly the degraded moment and is explicitly skipped, with the reasoning recorded). Auth and CORS posture are intake item c; until answered, the design assumes same-origin-or-CORS-with-credentials and flags that a cross-origin endpoint would preflight every POST (fine for budget one; budget two's constraint is already priced above).
...
2. **Three gating facts are still open** (intake a–c: payload sizes, redaction constraints, endpoint particulars). Every dependent conclusion above is marked provisional; the config seeds and the gzip decision cannot finalize until they answer.
3. **The projection-cost figures are proxy-hardware twice over** — desktop V8, inert literals. If the spike's on-device numbers fail the budget, plan B (capture-turn shallow snapshot + idle serialization) is named in question 3, and it re-admits a bounded mutation window; that trade would need its own ruling.
...
5. **The nets under-report by construction** on the Firefox arm (no Reporting API at the floor; crash-shaped deaths produce nothing) and before the pre-arm stub loads. Absence-based detection is backend work keyed on this report's identity fields; if the backend never builds it, those deaths stay dark and this report has said so.
...
**Assumptions in place of facts** (each resolves to an intake item):

- MQTT payloads and REST bodies are few-KB typical / low-tens-of-KB worst case (item a) — sizes the worked defaults and truncation thresholds.
- The worked bucket table's counts and byte budgets are placeholders (item d), as is the assumption that the app's xstate machines carry an id and may not carry a `version` — RD-7 records whichever exists.
- Outbound MQTT publish volume is a small fraction of the ~50 msg/s aggregate (no intake item raised it; the app profile records no inbound/outbound split) — the assumption behind question 5's decision not to pay for the outbound capture gap. If outbound volume is material, that verdict should be revisited before the emission route is dismissed.
- The app's CSP permits the pre-arm stub, or a nonce can be injected into the served `index.html` (raised for intake, not yet lettered) — without it the boot-window compensation silently does not exist.
- Raw payloads may not leave the device verbatim; the conservative structural-skeleton redaction default applies until item b answers.
- The report endpoint is same-origin or CORS-enabled, accepts tens of MB, and does not share fate with the MQTT broker (item c); gzip stays off until the endpoint confirms decoding.
- An app build identifier is available at runtime (item e); if not, adding one is flagged as a build change.
- Device CPU is embedded-class; the 10× de-rate stands until item f replaces it. The fleet's Firefox floor is ~124; at ≥133 the two-budget split relaxes and this report's delivery section should be re-read against `fetch keepalive`.
- No incumbent `onerror`/`unhandledrejection` reporters exist to chain with (item g); the nets install chaining-safe regardless (call-through to prior handlers).
- The MQTT clientId is readable by page code and permitted in reports (item i) — the device-identity assumption behind question 9.
- Tabs are typically one per device (item j); the design is correct at any N but duplicative.
- No application code runs in Web Workers (item k); `record()` is main-thread.
- Delivery interruption is rare (item l) — the assumption behind bounded-then-drop and the parking refusal.

## What a spike would validate

- **Projection cost on the real operand**: serialize live app-shaped objects (getter-bearing, Proxy-wrapped, immer-drafted) through the bounded projection inside 0060's dispatch path (the aedes harness exists), on hardware answering intake item f; accept/reject the eager-projection design against the ~3%-of-core budget, with plan B pre-named.
- **Boundedness as retention**: fast-check properties asserting the ring never exceeds `maxBytes`/`maxCount`, that the four record-loss species and the three interference counters reconcile against I4's equation, and — via WeakRef/GC probes — that truncated records hold no reference to their parent strings (the SlicedString hazard). Includes the two unbounded-allocation paths the enumeration found outside the rings: capped `unknownBuckets`/`configErrors` under a dynamic-name flood, and the byte-capped quarantine ride-along under oversized rejects.
...
- **Real-browser lane (deferred to 0120's harness, per the 0150 precedent for naming unreachable checks)**: net arming/chaining, the pre-arm stub's queue-and-replay, `<RecorderErrorBoundary>` behaviour in production builds, and visibilitychange-hidden flush timing. Renderer-crash deaths and `Reporting-Endpoints` delivery are unreachable from any harness this repo owns and stay unverified.
