...
This track examined how the application gets a flight recorder: a memory-bounded capture of recent xstate transitions, MQTT messages, HTTP exchanges, and custom event streams, held per bucket in RAM, and sent to the team's own endpoint when a trigger fires. We compared a built recorder kit against three open-source SDK cores behind a facade, and against an extension of the 0050 logging facade. The app facts arrived while this report was being written and it was revised against them: the 100 requested MQTT messages cost about 1 MB and buy about two seconds of history at the fleet's peak message rate, so the report recommends a deeper MQTT buffer; record sizes and event rates for the other three buckets were not supplied, so their share of the budget and the width of the interleaved window are still estimates. The endpoint is a separate cloud service, so a report can leave a device whose transport is broken. And the fleet runs Firefox 124 only, so no request survives page dismissal except a small beacon.

We recommend **build**: an owned recorder kit behind a small capture interface, adopting tracekit for stack parsing, adopting safe-stable-stringify **conditionally** (question 3 found its bounds cannot be observed from outside, so the spike settles whether the walk is owned instead), and copying several designs (Sentry's client-report loss counters, 0050's remote-sink backoff idiom, @statelyai/inspect's serialization recipe). The survey found that the hard parts — byte accounting, per-bucket rings, serialize-at-capture projection, the seal protocol, the trigger discipline, the two-budget delivery, and the owned payload schema — are owned work under every posture, and the strongest adoptable core holds live caller references in its history buffer, which the capture-safety analysis forbids. The most important risk is coverage, not code: renderer crashes produce no report at all on this fleet, and the only compensation is backend absence detection that nobody has committed to build. Two more follow — the owned capture code runs inside other modules' latency budgets, and what may leave the device is still unruled, because the team has a scrubbing tool but nobody has stated a policy. The next step is a spike that measures projection cost on the real dispatch path; the design-level failure-mode enumeration beside this report names the hazards it must exercise.

**As of**: 2026-08-23 (versions evaluated are listed per candidate)
**Recommendation**: build — owned recorder kit behind the capture interface in Key question 6; tracekit adopted under D-0027 rung 1 and safe-stable-stringify adopted conditionally on question 3's elision finding; Sentry core retained as design prior art, not a dependency
...
5. **The two thirds of the walk hazard this contract does *not* close, named rather than implied** (added 2026-08-23 at the gate). The plan's hazard is that the walk executes app code which can "throw, stall, or mutate"; points 1–4 close the throw. **Stall**: a getter or Proxy trap that blocks rather than throws is invisible to point 1's try/catch, produces no drop counter, and stalls the host's dispatch turn — the same shape as question 11's redaction-backtracking hazard and answered the same way, by measuring the *tail* rather than the mean at `maxEntryBytes` (added to question 15). There is no cheap client-side bound: a synchronous walk cannot be timed out from inside itself, so this is a residual hazard, and the honest mitigation is that the projection's operand is a bucket-scoped scrub output rather than an arbitrary graph. **Mutate**: the walk reads live app objects, so a memoizing getter or a reactive-store proxy changes app state by being observed, and a `project()` row is handed the caller's live entry and could mutate the payload 0060 is still dispatching to its other consumers. I6's "observation only" is therefore a claim about the recorder's *outputs*, not about the walk's effects on its operand — stated that way in the interface below — and `project()` carries a documented purity contract no validator can enforce. Both were raised as `UNOWNED` in the design-level enumeration (capture-path-14 and -16) and were not among its 28 amendments; naming them here is what this report can honestly do at survey depth.
...
- **Aborted REST requests: partially closed by the HTTP bucket itself, counter still recommended.** D-0018's ruling stands — aborts raise no telemetry envelope, visibility via a "boundary stats counter" that remains ratified language with no design (DECISIONS.md:147-152). The two-record HTTP bucket narrows this gap materially without touching the boundary: a request-record with no outcome-record at seal *is* the abort/in-flight evidence, in the bundle, with RD-4 marking. What the bucket cannot see is aborts of requests made outside the wrapped mutator (there should be none, by D-0002's restricted-imports discipline) and abort *reasons*. The report therefore recommends 0160 supply the missing design as a proposal to 0060's owners rather than building around it: the stats counter as three monotonic integers on a **new** read-only `stats: { aborted; timedOut; settled }` member of the boundary handle — or as three fields alongside `BoundarySnapshot.depths` on wire 2, which is the nearer precedent (corrected at the gate 2026-08-23: an earlier draft called this "the existing `stats` surface", and 0060's accepted handle carries no such member; the phrase "boundary stats counter" is ratified language, not a shipped surface, which is what the same sentence already said) — read at bundle-assembly time by the transport adapter into the wire body's environment block — the same route question 9 uses for `IngressStats`, which keeps the recorder observation-only — with no new events and no telemetry, inside D-0018's letter. Whether to build it is 0060's call; the recorder is complete without it, just blinder to non-mutator aborts that lint says should not exist.
...
//     the quarantine ride-along carries its own byte cap (quarantineMaxBytes) because
//     it is pulled at seal rather than metered per record on the capture path.
...
//     continued capture.
// I6  Observation only, IN ITS OUTPUTS. The recorder subscribes to nothing, publishes
//     nothing, classifies nothing, participates in no transaction, and never reports
//     through itself. It reads only what the composition root injects (the taps' own
//     record() calls and config.quarantineSource) — never a host surface it went and
//     found. The qualifier "in its outputs" is load-bearing and added 2026-08-23: the
//     projection walk READS live app objects, so a memoizing getter or a reactive
//     proxy changes app state by being observed, and a project() row is handed the
//     caller's live entry (question 3, point 5). That is a residual hazard with a
//     documented purity contract, not an invariant this interface can enforce.
//     The injected transport MUST NOT route through any fetcher a tap wraps
...
export interface RecorderConfig {
  ...
  /** The two other terms of I3's formula, which had no config home until the gate
   *  found the validator's inputs were partly imaginary (added 2026-08-23).
   *  headerAllowance reserves bytes for the bundle header — identity, the health
   *  snapshot, configErrors, the trigger's parsed stack — which no bucket meters;
   *  quarantineMaxBytes caps the ride-along, which enters the bundle at seal rather
   *  than through the per-record meter. Both are stated defaults a config may raise:
   *  headerAllowance 256 KiB, quarantineMaxBytes 1 MiB. The spike measures whether
   *  the header default is generous or thin; PER_RECORD_OVERHEAD is a measured
   *  CONSTANT rather than a config field, because a config that could set it could
   *  defeat the envelope check by declaring it zero. */
  headerAllowance?: number;
  quarantineMaxBytes?: number;

  /** The MQTT quarantine ride-along, injected rather than fetched (I6). When the
   *  composition root supplies it, the SEAL pulls, projects, redacts, meters and
   *  caps the entries like any other evidence — which is what makes question 13's
   *  whole-bundle redaction and question 15's byte-cap property true of this section
   *  too. Omitted: the bundle carries no quarantine block. */
  quarantineSource?: () => readonly unknown[];
   ...
   *  digest is not redactable.) NOTE the divergence from 0050, stated rather than
   *  glossed (2026-08-23): 0050's accepted sketch types patterns as live `RegExp[]`
   *  with no ruleId, which is the lastIndex hazard below and cannot carry RD-10's
   *  markers. This report PROPOSES the same amendment to 0050 rather than assuming
   *  one shared value already exists (question 13). */
...
export interface BucketPolicy {
  ...
  /** The one exemption from `redaction.mode: 'skeleton'`, and it is a POLICY BIT,
   *  not a per-bucket rule set — so question 6's refusal of per-bucket rule sets is
   *  untouched (added 2026-08-23 at the gate). True: this bucket's leaf values ride
   *  verbatim, subject to the shared rules. Default false. A bucket carrying app
   *  data cannot be exempted by accident, and the exemptions are visible in one
   *  place — the config digest every bundle header carries. */
...
export interface RedactionRules {
  /** THE default posture, and the operand risk 2's mitigation needs (added
   *  2026-08-23 at the gate — the report stated a conservative default that no
   *  config value could express, and then worked an example configuring the
   *  opposite). 'skeleton': every leaf value of a bucket carrying app data is
   *  redacted, keys kept, each stamped {redacted, ruleId: 'default-skeleton'}
   *  (RD-10) — the shape that holds until someone with the authority rules on
   *  disclosure. 'denylist': only matching keys/paths/patterns are redacted, which
   *  is what a ruled policy would license. DEFAULT 'skeleton', because the failure
   *  direction of the wrong default is a payload leaving the device under a policy
   *  nobody wrote. */
  mode?: 'skeleton' | 'denylist';
  keys?: string[];
  paths?: string[];
...
export interface SealedBundle {
   ...
   *  2026-08-23: Q4 promised this section and no type carried it). It is filled AT
   *  SEAL by the recorder, pulling `config.quarantineSource` — an injected supplier,
   *  which is what I6 permits and what keeps this section inside the redaction rules
   *  and the byte cap (corrected 2026-08-23 at the gate: filling it from the
   *  transport adapter after the seal placed the one section no projection scrubber
   *  has touched outside the only component that owns a rule set and a meter).
   *  It is BYTE-CAPPED at `quarantineMaxBytes` and marked, because 0060's ring is
   *  bounded at 100 entries by COUNT and holds schema-rejected traffic whose whole
   *  failure mode is being malformed or oversized: uncapped it would put unmetered
   *  megabytes into a bundle every counter reads as in-envelope. */
  readonly quarantine?: {
...
const recorder = createFlightRecorder({
  ...
  quarantineSource: () => boundary.quarantine.entries(),   // pulled at seal, not pushed
  redaction: {
    // mode omitted = 'skeleton': leaf values redacted, keys kept, every one marked.
    // This is the shipping default until someone rules on disclosure (risk 2), and
    // the worked table shows it rather than the permissive shape a ruling would
    // license (corrected 2026-08-23 at the gate — the example configured the
    // opposite of the default the report says it ships).
    keys: ['password', 'token'],       // still exact: named keys redact under both modes
  ...
  buckets: {
    ...
    //
    // http/xstate/log maxBytes are WORST-CASE RESERVES, not expected fills: at the
    // analogy record sizes in Q2, each expects to hold well under a quarter of its
    // budget. Tightening those three toward 1 MiB / 1 MiB / 0.25 MiB would free
    ...
    // rather than this table guessing twice. The MQTT row is the EXCEPTION: its byte
    // cap is sized to the typical fill, so it binds before maxCount whenever payloads
    // run large (added 2026-08-23 at the gate).
    //
    // I3's residual, printed because the table sits 5% from a table-level validation
    // error and a tuner cannot otherwise see the margin: Σ maxBytes = 19 MiB against
    // envelopeBytes 20 MiB, so 1 MiB (1,048,576 B) is left for headerAllowance plus
    // PER_RECORD_OVERHEAD × Σ maxCount (1,400 records) — the table validates while
    // headerAllowance + 1,400 × PER_RECORD_OVERHEAD ≤ 1,048,576 B. At the 256 KiB
    // header default that is ≤ 561 B per record, against a "tens of bytes" estimate:
    // roughly an order of magnitude of margin, and the spike measures both terms.
    mqtt: {
      // ~24 s at ~50 msg/s AT ITEM a's 10 KB TYPICAL PAYLOAD. The 12 MiB cap binds
      // first above that: at the ~50 KB worst case this row holds ~250 messages,
      // about 5 s. The requested 100 was ~2 s at either size.
      maxCount: 1_200,
...
let lastConnection = boundary.actor.getSnapshot().connection;
const stopWire2 = boundary.actor.subscribe((snap) => {
  ...
  //
  // Corrected 2026-08-23 at the gate: the first draft tested
  // `snap.connection === 'reconnected'`, a value 0060's accepted union does not have
  // ('idle' | 'connecting' | 'connected' | 'reconnecting' | 'degraded' | 'ended',
  // 0060 spike design.md:296) — dead code whose failure is invisible, since a bundle
  // with no gap records reads exactly like a session with no reconnects. The gap is
  // a TRANSITION, not a state, and the adapter must compare against its own last-seen
  // value because 0060's O6 permits snapshot notifications to coalesce.
  if (snap.connection === 'connected' && lastConnection !== 'connecting') {
    mqttTap.record({ gap: true, from: lastConnection }, { kind: 'gap' });
  }
  lastConnection = snap.connection;
});
...
- **Cost containment, not only throw containment** (added 2026-08-23). The enumeration found a capture-path step this report had not budgeted — redaction — and question 3's point 5 names the other one it still cannot bound, a getter or Proxy trap that stalls rather than throws. Config-supplied patterns run per record inside the capture turn against bodies up to `maxEntryBytes`, and a pattern with nested quantifiers backtracks superlinearly — a stall, not a throw, so the try/catch never sees it and no drop counter records it, while 0060's shared dispatch turn sheds for every consumer. Three answers, all cheap: patterns are data with `g`/`y` rejected (question 6), the applier scans the bounded projection rather than raw input, and the spike measures redaction cost at `maxEntryBytes` alongside projection cost — a rule set is app config in an agent-authored codebase, so it must be treated as untrusted input to a hot path.
...
| Runtime overhead (high) | strong — ~6.5 KB gzip *of deps only*, owned kit unquantified; capture cost owned and budgeted | adequate — ≤33 KB gzip for the *error-only subset* (its own CI budget); steady-state fine | adequate — 15.1 KB gzip *whole package*, moot given the ceiling | adequate — 34.4 KB gzip *whole package* incl. web-vitals/ua-parser; **this figure carries no source in this survey** and should not be read against the others without one |
...
**Three** rows carry an **evidence asymmetry** worth naming, because the verdict must not lean on them (the third added at the gate 2026-08-23, where the paragraph whose job is to stop the verdict leaning on measurement artifacts was found certifying as symmetric the row carrying the largest one). Browser compatibility scores the build "strong" because this survey floor-checked each primitive it uses while the three SDKs are marked "not re-verified here". Runtime overhead scores the build on its dependency bytes with the owned kit unquantified, against three package figures computed under three different counting rules. And **Boundedness scores an unbuilt design's stated intentions against three shipped implementations' verified behaviour** — the build's own cell concedes it ("the claim is construction plus property tests, and stays provisional until the spike runs them"), and under D-0001 the build can never have source evidence, so the asymmetry is structural and unavoidable; what is avoidable is calling it symmetric. The criterion is also this track's own proposal (D-0041), worded around the design goals the build posture holds. **The recommendation therefore rests on capture-safety**, where the evidence genuinely is symmetric and read from each candidate's own source; Boundedness carries corroboration rather than weight until the spike runs.

## Recommendation

**Build: an owned recorder kit** — per-bucket byte-and-count-bounded rings of capture-time-serialized records behind the capture interface in Key question 6, the trigger protocol of question 7, the two-budget delivery of questions 9–10, and a vendored OpenAPI payload contract generated through 0010's pipeline. Adopt **tracekit 0.4.9** outright (dual-engine stack parsing, D-0027 rung-1 justified in the Survey). Adopt **safe-stable-stringify 2.5.0 conditionally**: question 3 found that its `maximumDepth`/`maximumBreadth` bounds cannot be observed from outside — a `replacer` never sees breadth elision because the library sorts and slices before iterating, and its in-band marker strings are indistinguishable from app strings with the same content — so the wrapper owns the walk, and what remains of the dependency is cycle handling that the same argument makes hard to mark. Whether that residual still clears rung 1, or the walk should simply be owned, is what the elision-mechanism spike settles; question 15 calls it the finding that most affects this bill of materials, so the recommendation carries the condition rather than presenting the adoption as decided. Copy three designs with citation: Sentry's client-report loss accounting, the loglevel-plugin-remote backoff idiom 0050 already copied, and @statelyai/inspect's serialization recipe. Own the ring (~60 lines; mnemonist shelved as fallback), the seal, the bundle assembly, and the schema.

**Rationale.** The charter held build and adopt+wrap at equal merit, so this verdict rests on what the survey verified rather than on a default. Three findings decide it. First, the hard requirements are owned work under every posture — no surveyed library bounds memory by bytes, none serializes at capture, none carries per-bucket taxonomy, and the payload schema is ours by charter — so adoption cannot shrink the risky code, only relocate the plumbing around it. Second, the strongest adoptable core fails the capture-safety requirement at its center: Sentry's ring stores caller references and defers normalization to event-prep, which is precisely the mutation-unsafe, graph-pinning capture the taps forbid; fixing it from outside reduces the SDK's contribution to nets, plumbing, and counters — each small, each already copied or copyable. Third, the wrap's residual costs are not hypothetical: a dummy DSN to make the SDK function at all, suppression of default integrations that would double-instrument 0060/0070's governed surfaces, a global `Error.stackTraceLimit` write, and major-version churn. The build's owned surface, costed rather than waved at (corrected at the gate 2026-08-23, where "~a few hundred owned lines" was found to omit every piece this report calls non-trivial): roughly **~250 lines of nets and pre-arm stub** and **~60 lines of ring**, plus the seal, the bundle assembly, the redaction applier, the config validator, the delivery loop's four rules, the four tap adapters — and the whole serialization walk if the elision spike goes the other way. Its **property-testable core** — the ring's byte invariants, the drop accounting, seal correctness — is exactly what the accepted fast-check lane tests best. Its largest costed chunk is not: the nets and the stub need a real browser, are named among question 15's unreachable claims, and wait on 0120's harness, which has not started. That is the honest shape of the build's risk, and it is why the spike list separates what a lane can reach from what cannot.
...
1. **The owned capture code is the weakest link, and it runs in other modules' disciplined paths.** Mitigation, stated accurately (corrected at the gate 2026-08-23 — the first version claimed "every piece copies a cited design", which contradicts the survey finding the whole build verdict rests on): the pieces **with** prior art copy it with citation — the loss counters, the backoff idiom, the xstate serialization recipe — and the pieces at the centre of this risk have none, because the report's own case for building is that no surveyed library bounds memory by bytes or serializes at capture. The byte meter, the byte-eviction loop and the seal protocol are novel, which is exactly why their properties are fast-check lanes rather than review promises. Beyond that: the interface isolates taps behind their own throw containment, and the spike measures the real operand (live app-shaped objects) on device-class hardware before adoption.
...
6. **The recorder can degrade its neighbour through memory alone** (added 2026-08-23; ownership ruled by **D-0043**). The plan separates this track from 0150 on the grounds that they meet only if durable parking is adopted, and question 10 refuses parking — but on a RAM-disk device IndexedDB *is* RAM, so 0150's inbox store, its dedup keys, and its transactions compete for the same physical budget as this recorder's rings and its held bundles, in every tab. IndexedDB writes fail under storage pressure, and 0150's central claim is that the effect and the message identity commit together, so the recorder can break a durability guarantee it never touches — through a channel neither track's boundary analysis covers, since "must not degrade the app" had been checked against latency and throw containment and not against memory as a shared resource. **D-0043 disposes of the ownership question this risk raised**: each track states and bounds its own budget and models no sibling's. This track complies as written — the envelope is a config field, validated statically at construction (I3), and 20 MiB per tab is a stated number rather than an emergent one — and 0150 owes the same for its store. What the ruling deliberately does not do is own the **sum**: per-track budgets compose only by addition, and no artifact adds them. So the residual is arithmetic rather than analysis — and the arithmetic has to count everything this design holds at once, which at the gate (2026-08-23) it did not. **Steady state**: ~40 MiB of serialized ring bytes per device (20 MiB × 2 tabs), nearer 80 MiB of heap by question 2's UTF-16 caveat — about 1.3% of a 6 GB device. **Transient peak**: question 10 keeps one bundle in flight plus one pending per tab, and question 9 budgets a seal peak of envelope-plus-largest-bucket; a device where both tabs hold both delivery slots against a slow endpoint carries up to ~76 MiB of sealed-but-undelivered bundles on top of the rings — ~116 MiB serialized, nearer **~230 MiB of heap, about 3.8% of a 6 GB device and 1.4% of a 16 GB one**. That is the number the residual should be read against, and it is still comfortable, but it is not "under 2%" and the difference matters precisely because the transient arrives during an incident, when the app is also under stress. Recorded rather than mitigated, and it reopens if either budget grows by an order of magnitude or a third memory-holding component appears.
...
**Facts, and what remains assumed.** Intake items a–l were answered on 2026-08-23; items a–k are recorded in facts/app-profile.md and item l's "nobody has measured" stands in the intake file (added to the profile at the gate). This report was revised against them rather than shipping its placeholders. Items **m** (the app's CSP) and **n** (outbound MQTT volume) were added the same day from the design-level enumeration and remain **open**, and the gate added **[intake 2026-08-23-0160-endpoint-contract](../../intake/2026-08-23-0160-endpoint-contract.md)** for three asks this report generated and never registered — so the intake register is not closed. **Six of the twelve answers settled less than their question asked** (items a, b, c, e, h, i — four of them initially read as closed, which is the evidence D-0042 was minted on), and item d's xstate half and item j's truncated clause went unanswered too, which is why the list below runs to more than six bullets. The counting rule is printed because the gate found this number stated as "four" against the intake register's six. What they settled, and where each one landed:
...
| Two tabs, distinct views (j) | Multi-tab is a live condition: 2× envelope per device, and question 16's liveness answer is load-bearing. **The answer is truncated mid-clause; what else distinguishes the tabs is unread** |
...
- **The report endpoint deduplicates rather than appends**, on `(deviceId, pageIncarnationId, bundleId)` — added at the gate 2026-08-23 as an undeclared assumption the design-level enumeration had already flagged. Three rulings rest on it: at-least-once retry with bounded-then-drop (question 10), the acceptance of up to N near-duplicate bundles instead of cross-tab coordination (question 16), and the durable-parking refusal. If the endpoint appends — the common shape for a log intake, and it is a service still being built — every ambiguous delivery outcome stores a duplicate, every two-tab device stores near-duplicates, incident counts inflate, and the repair is a backend schema change made after the vendored contract froze. Raised as **[intake 2026-08-23-0160-endpoint-contract](../../intake/2026-08-23-0160-endpoint-contract.md)**.
- **The report endpoint will expose a route and a parser for the ≤ 64 KiB loss marker** — same intake file. A beacon's response is unobservable, so if it does not, the entire compensation for refusing durable parking is silently absent and the client can never learn it.
- **The endpoint ceiling will be negotiated in wire bytes, not ring bytes** — same intake file. Question 2's inflation factor is the reason the two differ by roughly 1.4–1.7× before headers; a ceiling agreed against the ring figure is agreed against the wrong number.
- **xstate transition, REST request, and log-line rates** — never supplied, so three of the four buckets' time spans are unknown and with them the width of the interleaved window question 1 calls this recorder's one distinct claim. If any is much faster than assumed, the raised MQTT ring interleaves with a much shorter window from the others; the repair is a per-bucket span target rather than a count, settled by the spike's field bundles.
- **How the two tabs differ beyond their views** — item j's answer is truncated mid-clause ("distinct views and distinct ___"). If they carry distinct sessions, logins or clientIds, question 16's identity reasoning and the single-connection invariant both move; the report reads only the half that was written.

## What a spike would validate

- **Projection cost on the real operand**: serialize live app-shaped objects (getter-bearing, Proxy-wrapped, immer-drafted) through the bounded projection inside 0060's dispatch path (the aedes harness exists), on device-class hardware (item f: desktop-class, low power); accept/reject the eager-projection design against the ~3%-of-core budget, with plan B pre-named. **Measure the tail, not the mean** (added at the gate): question 3's point 5 names a getter or Proxy trap that stalls rather than throws, which no try/catch and no drop counter can see, so the number that decides the design is the p99 walk at `maxEntryBytes`.
...
- **Redaction as a hot-path cost and a correctness property**: measure the applier at `maxEntryBytes` with a realistic rule set (the cost the first draft never budgeted), assert that a `g`/`y` pattern is rejected by the validator rather than silently skipping alternate records, assert markers appear exactly where rules fired, and assert the **default posture**: with `mode` unset, a bucket without `valuesVerbatim` emits no leaf value at all (added at the gate — the conservative default risk 2 rests on had no expression in the interface and no test).
...
- **Per-record overhead, the header allowance, and the wire-inflation factor**: measure `PER_RECORD_OVERHEAD` and `headerAllowance` — both terms of I3's formula, and the worked table validates only while their sum stays under ~1 MiB across 1,400 records — plus the quarantine ride-along's real byte cost, and the ring-bytes-to-wire-bytes ratio for a representative bundle, since the endpoint ceiling is negotiated on the latter.
