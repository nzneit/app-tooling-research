...
The wrapper ecosystem is a graveyard — async-mqtt dormant since 2022 (obsoleted by mqtt.js v5's first-party `*Async` APIs), paho-mqtt's last npm publish 2018, precompiled-mqtt dead, mqtt-react-hooks the wrong altitude for a validation choke point (sources under "mqtt.js and the build's prior art" in Sources). Nobody who needs a serious browser MQTT boundary found anything to adopt; they all wrote the thin owned wrapper. Nothing eliminates the build: its risks are authoring cost and unproven glue, not license, maintenance, or capability gates. **Selected — the recommendation's core.**
...
### Wave 2 completeness scan — challenger and residual finds

> **2026-08-17 terminology update (no claim changed).** This section was written as "Wave 2 gap
> scan". Ruled at the 9900 gate (intake 2026-08-15 item f): **gap sweep** is the cross-cutting
> adversarial pass over a wave's finished reports that hunts for falsified claims, and the
> within-track check for candidates a survey never considered is now the **completeness scan** —
> "gap scan" is retired, because one word doing two jobs was the ambiguity. The rename was
> applied throughout this report; nothing else about it was edited.
...
- **A-5**: QoS 0/1 with `clean: true` sessions — keeps the worst resubscribe/duplicate bug history out of scope; the dedup guard is retained regardless.
  > **2026-08-17 correction (in place; the original text above is left intact as the record). A-5 is FALSIFIED.** The user supplied the fact: the client runs **`clean: false` with a persisted clientId** — a persistent session. Four consequences, none of which reverse this report's recommendation but three of which change its scope. **(1) The exclusion A-5 bought is void**: a persistent session means the broker queues QoS 1 messages while the client is disconnected and redelivers on reconnect, so [#909](https://github.com/mqttjs/MQTT.js/issues/909) duplicate-delivery-across-offline-windows is **in** scope, exactly the history A-5 declared out of it. Risk 2 in "Risks and unknowns" should be read at its higher weight. **(2) The spike's design.md hard-codes the wrong value**: `readonly clean: true; // A-5: clean sessions, fixed` (spikes/boundary-wiring/design.md) is a fixed config constant, not prose — anyone implementing from it would ship the wrong session mode. The `clean` flag is not a fixed value and belongs in config. **(3) Key question 5's delegation list needs one revision**: `resubscribe: true` is redundant against a retained session, and because [MQTT-3.3.1-6] requires retained messages to be re-sent on every new subscription, resubscribing re-triggers retained delivery on every reconnect. — **2026-08-18, corrected on the same day it was written: this is wrong for this deployment.** The broker is ActiveMQ Classic, which creates a durable subscription only for QoS ≥ 1; QoS-0 subscriptions carry no session state and are **not** restored on a `clean: false` reconnect. `resubscribe: true` is therefore **required**, not redundant, or the client goes silently deaf on every QoS-0 topic after its first reconnect. The retained-delivery consequence still stands as a cost of resubscribing. Recorded rather than quietly fixed, because the error is instructive: the claim was inferred from what the MQTT specification permits a broker to retain, without checking what this broker does — the same reasoning-from-protocol-instead-of-implementation that AMQ-7045 punishes. — **2026-08-18, second amendment, same day: this correction is ActiveMQ-conditional and must not be read as settled.** The user reports a likely migration to **Eclipse Mosquitto on a 3-to-6-month horizon** (D-0039). Mosquitto restores *all* subscriptions on a `clean: false` reconnect per §3.1.2.4, including QoS 0, so `resubscribe: true` becomes **redundant again** after the swap — the original text was right for a spec-conforming broker and wrong only for this one. The setting is therefore not a fixed value in either direction: it is **required on ActiveMQ Classic, redundant on Mosquitto**, and belongs in configuration beside `clean`, per consequence (2) above. That this claim has now flipped twice in two days is itself the finding, and the transferable rule is narrower than "check the implementation": **a claim about what a broker retains is a claim about one product and one version, and must be written with that product named in the sentence.** Whether this reopens **D-0015** remains a gate decision. **(4) The in-memory dedup guard is now load-bearing across a boundary it cannot cross**: it keys on MQTT packet identity (A-8), and Packet Identifiers are per-session slots released for reuse on acknowledgement, so the guard cannot recognise a redelivery that arrives after a page reload — which is precisely the case `clean: false` creates. Track **0150-message-inbox** (D-0038) owns that gap. Not corrected here because it is not this report's call: whether this reopens **D-0015** is a gate decision.
...
All accessed 2026-08-14. Primary evidence was gathered in nine per-candidate investigation memos during the survey (rxjs, effect, tanstack-query, openapi-fetch, ts-rest/zodios, xstate-actors, emitters, prior-art, completeness-scan — transient working files, since retired); the URLs below are their consolidated source set and the durable record. Binding upstream: tracks/0010-contract-pipeline/report.md (accepted); plan: tracks/0060-transport-abstraction/research-plan.md; app facts: facts/app-profile.md (unfilled — assumptions A-1…A-14 per D-0004).
