...
// ── REST contract (per-status schemas) ───────────────────────────

/**
 * Structural stand-in for a zod schema — the REST leg's per-status validator.
 * Deliberately structural: zod is category 1 (in-process, no seam) and the
 * schemas arrive as data, so nothing below the seam imports zod. A schema
 * generated by orval's `client: 'zod'` output satisfies this as shipped.
 */
...
/**
 * One row per declared operation, keyed `'METHOD /path/{param}'` — the shape of
 * an OpenAPI `paths` entry, flattened. The row mirrors the operation's
 * `responses` object: every declared status mapped to its schema. Statuses (and
 * paths) absent from the table are UNDECLARED.
 */
export type RestContract = Readonly<Record<string, Readonly<Record<number, ResponseSchema>>>>;
...
export interface BoundaryConfig<P extends PolicyTable> {
  ...
  rest: {
    ...
    headers?: HeadersInit;
    /**
     * The REST leg's analogue of the policy table: every DECLARED response of
     * every declared operation, with the schema its body must parse against.
     *
     * Addition to design.md's `rest` config shape (recorded in findings.md).
     * design.md fixes the BEHAVIOUR — class 3 is "a declared non-2xx body that
     * parses against its per-status schema", class 4 is `undeclared-status` —
     * but never says how the per-status schemas reach the single REST ingress.
     * They arrive here as data, exactly as the compiled Ajv validators do on
     * the MQTT side, so the ingress imports no generated code (I5).
     *
     * Absent or unmatched is not a loophole: with no declared schema for the
     * status, nothing can be validated, so the response is class 4
     * `undeclared-status` (I1 — nothing unvalidated crosses the interface).
     */
