...
- **design.md's one-line orval binding is wrong as written, and the spike
  ...
  for free. (The compile proof types a stand-in `declare function` plus a
  hand-written object literal that demonstrates this same type rule, not the
  real generated call site compiled against the naive mutator.) **The hazard is
  one step later**: hit TS2353, widen the request
