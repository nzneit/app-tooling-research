...
- **Where `signal` lands depends on `output.httpClient`, and the fetch
  ...
  asserts all of it rather than glossing it.
...
- **The layering lint is a real mechanism, and its footgun is now documented.**
  ...
  the caveat on this oxlint version (the 0070 spike found it too), which
  upgrades it from "0010's open question" to a settled repo-wide practice.
- **Open cross-track item — abort normalization, for the owners of
  ...
  `reason: 'aborted'` error; its optimistic-mutation kit then returns
  `{outcome: 'cancelled'}` rather than a rollback. Both readings are correct at
