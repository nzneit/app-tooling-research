...
Suite: **54 tests, 11 files, all green**; `npm run typecheck` clean. The
...
- **Still open for the next task:** signal threading through orval's generated
  ...
  declared-status table, the QueryCache `onError` telemetry tap, the oxlint
  layering rule, and the mqtt-pattern adopt-vs-vendor recommendation.
